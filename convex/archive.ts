...
export const fetchUserContent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, "archive:fetchContentWorker", { jobId });
...
export const fetchContentWorker = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation("archive:updateJobStatus", {
      jobId: args.jobId,
      status: "running",
    });

    try {
      const job = await ctx.runQuery("archive:getJob", { jobId: args.jobId });
      if (!job) {
      ...
      for (const platform of job.platforms) {
        if (platform === "ea-forum") {
          ...
          await ctx.runMutation("archive:saveContent", {
            jobId: args.jobId,
            content: content,
          });
...
export const exportToCsv = action({
  ...
  handler: async (ctx, args) => {
    ...
    const content = await ctx.runQuery("archive:getJobContent", { jobId: args.jobId });
